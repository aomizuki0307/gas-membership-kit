...
![E2Eデモ: テスト決済 → 自動起票 → 退会反映](docs/images/demo-e2e-join-cancel.gif)

静止画で確認する場合。Payment Link をテストカード 4242 で決済すると、Webhook 経由で会員DBに自動起票される。
...
実測済み: 宛先なしフォールバック（運営チャンネルへ `[DMテスト代替]`）→ DM 実送信 → 同月再実行で「送信=0 / 済みスキップ=1」の冪等性まで全経路を確認。バッチ再実行時の実行ログ（対象=3 のうち 送信=0 / 済みスキップ=1 / 宛先なし=2。二重送信ゼロ）:

![バッチ冪等性の実行ログ](docs/images/demo-slack-batch-idempotency.png)

検証ゲートの単体テストログ（ドメイン様文字列・IP・制御文字・bidi・ゼロ幅をすべて拒否し、メンション構文を無害化）:

![検証ゲートのテストログ](docs/images/demo-slack-gate-test.png)
...
毎月1日 11時台 JST の時刻トリガー（9時=レポート生成、10時=Slack配信の後）が `rebuildKpiDashboard` を起動し、KPI シートに月次集計表（会員数・新規入会・退会・チャーン率・継続率）とチャート3枚を全再構築する。GASエディタからの手動実行でいつでも再計算できる。実行から全再構築までの実挙動（上の E2E デモで発生させた入退会が集計に反映される瞬間）:

![KPI再構築デモ](docs/images/demo-kpi-rebuild.gif)

![KPIシート](docs/images/demo-kpi-sheet.png)

集計表の右にはチャート3枚（月次 入会・退会数 / アクティブ会員数の推移 / 継続率の推移）が並び、毎回の実行で削除→再挿入される:

![KPIチャート](docs/images/demo-kpi-charts.png)
...
- **ロックは書き込み区間（チャート削除→再挿入）だけ数秒握る**。rebuild 同士の並走（トリガー×手動）によるチャート重複を防ぐ。集計（読み取り）では握らない: webhook と共有のロックをバッチで握ると Stripe イベント喪失リスク（機能2と同じ理由）を作る。並走受信との読み取り競合は最悪「最新1イベントが今回の集計に入らない」だけで、次回実行で自己修復する
- **チャーン率はコホート定義: 月初アクティブ会員のうち当月退会した割合**。分子を全退会数にすると同月入会即退会が混ざって 100% 超（継続率マイナス）になりうるため、分子は月初在籍者の退会に限定し、構造的に 0〜100% に収める。月初0会員の月は 0/0 で未定義なので空欄にする（0 と書くと「解約ゼロの好調月」と区別できない）。継続率 = 1 − チャーン率
...
月次トリガーの設置状態（レポート9時・Slack10時の後の11時台）:

![KPI月次トリガー](docs/images/demo-kpi-trigger.png)
...
| churn_rate / retention_rate | コホート定義の退会率と継続率（月初0会員の月は空欄） |
