...
| `ANTHROPIC_API_KEY` | Claude API のキー `sk-ant-...`（[Claude Console](https://platform.claude.com/) で発行。機能②の月次レポートで使用） |

機能3 の着手時に `SLACK_BOT_TOKEN` を追加する予定。

## 5. シート初期化と設定確認

GASエディタで以下を順に実行（初回は権限承認ダイアログが出る）:

1. `initializeSheets` — Members / EventLog / Reports シートとヘッダー行を作成
2. `checkConfig` — Script Properties が揃っているか確認
3. `checkStripeConnection` — Stripe キー設定後に。`HTTP 200` が出ればOK
4. `checkClaudeConnection` — ANTHROPIC_API_KEY 設定後に。`ok` が出ればOK（数トークン分だけ課金される）

機能②（月次レポート）を有効にする場合は、続けて `setupMonthlyReportTrigger` を1回実行する（毎月1日 9時台 JST の時刻トリガーが張られる）。
